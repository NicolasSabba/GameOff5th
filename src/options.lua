...
local function change(amount, dt)
   if optionsMenu.opt == 0 then
       ...
       love.window.setMode( conf.ww, conf.wh, {fullscreen = conf.full, vsync = false})
       constants:Resize()
       complete.update()
...
function options:load()
    ...
    fw = front:getWidth('Save options')
    saveData = {}
...
function options:draw()
    ...
    if state == 1 then
        love.graphics.draw(saveData.canvas, ww - saveData.cw/2 + 16, wh - saveData.ch/2)
    elseif state == 2 then
        love.graphics.draw(complete.canvas, ww - complete.cw/2 + 16, 0)
    end
end
